...
% ========== LOAD DATA ===========
% ================================
PLOTNAME = 'Quadprog-TestCaseA';

filename_array = ["6_16_20_51","6_16_21_43"];
iscontrol_bool = [0,1];

% Start a figure
figure

for fileind = 1:length(filename_array)
    
    filename = filename_array(fileind);
...
%     for i = 1 : length(ani_posx)-1
    ...
    if iscontrol_bool(fileind) == 0
        plot(ani_posx, ani_posy, 'k.-'); % Plot the solution up until i
    else
        plot(ani_posx, ani_posy, 'k:'); % Plot the solution up until i
    end
    hold on
    ...
    % Plot initial position of spacecraft
    chaser = theChaserSpacecraft(ani_posx(1), ani_posy(1), ani_theta(1), ani_rc);
    patch(chaser(:,1), chaser(:,2), 'w', 'facealpha', 0.5, 'edgecolor', 'r') % Plot the chaser spacecraft
    hold on
    target = theTargetSpacecraft(ani_ptar(1,1), ani_ptar(1,2), ani_targett(1), rt);
    patch(target(:,1), target(:,2), 'w', 'facealpha', 0.5, 'edgecolor', 'k') % Plot the target spacecraft
    hold on
    obstac = theObstacleSpacecraft(ani_pobj(1,1), ani_pobj(1,2), ani_thetaobj(end), rt);
    patch(obstac(:,1), obstac(:,2), 'w', 'facealpha', 0.5, 'edgecolor', 'b') % Plot the obstacle spacecraft
    hold on
%     plot(KOZ*cos(angles)+ani_pobj1(1,1), KOZ*sin(angles)+ani_pobj1(istart,2), 'b-'); % Obstacle 1
%     hold on

    % Plot final position of spacecraft
    transparency = 0.1;
    chaser = theChaserSpacecraft(ani_posx(end), ani_posy(end), ani_theta(end), ani_rc);
    patch(chaser(:,1), chaser(:,2), 'w', 'facealpha', transparency,'edgealpha', transparency, 'edgecolor', 'r') % Plot the chaser spacecraft
    hold on
    target = theTargetSpacecraft(ani_ptar(end,1), ani_ptar(end,2), ani_targett(end), rt);
    patch(target(:,1), target(:,2), 'w', 'facealpha', transparency,'edgealpha', transparency, 'edgecolor', 'k') % Plot the target spacecraft
    hold on
    obstac = theTargetSpacecraft(ani_pobj(end,1), ani_pobj(end,2), ani_thetaobj(end), rt);
    patch(obstac(:,1), obstac(:,2), 'w', 'facealpha', transparency,'edgealpha', transparency, 'edgecolor', 'b') % Plot the obstacle spacecraft
    hold on
%     obsKOZ = plot(KOZ*cos(angles)+ani_pobj1(end,1), KOZ*sin(angles)+ani_pobj1(end,2), 'b-'); % Obstacle 1
%     obsKOZ.Color(4)=transparency;
%     hold on
    ...
    plot(1:length(ani_velx), ani_velx, '.-r','LineWidth', 0.5);
    hold on
    plot(1:length(ani_vely), ani_vely, '.-b','LineWidth', 0.5);
    hold on
    plot(1:length(ani_tar_velx), ani_tar_velx, ':r','LineWidth', 1)
    hold on
    plot(1:length(ani_tar_vely), ani_tar_vely, ':b','LineWidth', 1)
    %hold on
    ...
    plot(1:length(ani_omega), ani_omega, '.-k','LineWidth', 0.5);
    hold on
    plot(1:length(ani_tar_omega), ani_tar_omega, ':k','LineWidth', 0.5);
    ...
    plot(1:length(ani_ux), ani_ux, '.-r','LineWidth', 0.5);
    hold on
    plot(1:length(ani_uy), ani_uy, '.-b','LineWidth', 0.5);
    hold on
    plot(1:length(ani_tau), ani_tau, '.-k','LineWidth', 0.5);
...
saveas(gcf,strcat(strcat('Saved Data/',PLOTNAME),'_experimentcomparison.png'))
