...
% This function takes the data produced from running "main.m" and produces
...
clear ani_posx ani_posy ani_theta ani_velx ani_vely ani_omega ani_ux ani_uy ...
    ani_tau ani_targetx ani_targety ani_pobj1 ani_pobj2 ani_rc ani_wdock ...
    ani_targett ani_tar_velx ani_tar_vely ani_tar_omega ani_m1 ani_m2 ...
    ani_pdockx ani_pdocky com_totalcontrol com_totalcost ani_analysisstring ...
    ani_CPUstring myWriter movieVector frame
...
% for each component.
ani_posx = sol(1:9:end);    % 1 x (k+1)
ani_posy = sol(2:9:end);    % 1 x (k+1)
ani_theta = sol(3:9:end);   % 1 x (k+1)
ani_velx = sol(4:9:end);    % 1 x (k+1)
ani_vely = sol(5:9:end);    % 1 x (k+1)
ani_omega = sol(6:9:end)*180/pi; % converted to deg/s
ani_ux = sol(7:9:end);      % 1 x (k+1)
ani_uy = sol(8:9:end);      % 1 x (k+1)
ani_tau = sol(9:9:end);     % 1 x (k+1)
...
ani_ptar = solptar; % (k+1) x 2
ani_targett = solthetat;                % (k+1) x 1
ani_tar_velx = vtar(1)*ones(size(ani_posx));   % 1 x (k+1); m/s
ani_tar_vely = vtar(2)*ones(size(ani_posx));   % 1 x (k+1); m/s
ani_tar_omega = thetat_dot*ones(size(ani_posx))*180/pi; % 1 x (k+1); deg/s
...
for i = 1 : length(ani_posx)-1
    ...
    plot(ani_posx(1:i), ani_posy(1:i), 'b.-'); % Plot the solution up until i
    hold on
    plot(ani_targetx(:,i),ani_targety(:,i),'b--', 'LineWidth', 0.5) % Plot predicted solution
    hold on
    plot(KOZ*cos(angles)+ani_pobj1(i,1), KOZ*sin(angles)+ani_pobj1(i,2), 'r-'); % Obstacle 1
    hold on
    plot(KOZ*cos(angles)+ani_pobj2(i,1), KOZ*sin(angles)+ani_pobj2(i,2), 'g-'); % Obstacle 2
    hold on
    
    % Calculate endpoints of each of the entry cone hyerplanes
    entryconex1 = ani_pdockx(i) + rcone*sin(ani_targett(i)+thetah)/ani_m1(i); % point for entry cone 1
    entryconey1 = ani_pdocky(i) + rcone*cos(ani_targett(i)+thetah)*ani_m1(i);
    entryconex2 = ani_pdockx(i) + rcone*sin(ani_targett(i)-thetah)/ani_m2(i); % point for entry cone 2
    entryconey2 = ani_pdocky(i) + rcone*cos(ani_targett(i)-thetah)*ani_m2(i);   
    ...
    chaser = theChaserSpacecraft(ani_posx(i), ani_posy(i), ani_theta(i), ani_rc, ani_wdock);
    patch(chaser(:,1), chaser(:,2), 'w', 'facealpha', 0.5) % Plot the chaser spacecraft
    hold on
    target = theTargetSpacecraft(ani_ptar(i,1), ani_ptar(i,2), ani_targett(i), rt, ani_wdock);
    patch(target(:,1), target(:,2), 'w') % Plot the target spacecraft
