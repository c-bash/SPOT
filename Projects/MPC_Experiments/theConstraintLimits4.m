function [cl,cu,bineqvec] = theConstraintLimits4(Umat,Fmat,Wmat,con0,con,D1,D2,Dt,...
    ...
    bineqvec(con0(4)+1 : sum(con0(4:5))) = -ones(con0(5),1) - D2*(ro_2_9 + robj2_9);
    bineqvec(sum(con0(4:5))+1 : sum(con0(4:6))) = -ones(con0(6),1) - Dt*(ro_t_9 + rtar_9);
    
    if sum(con) > sum(con0) % if entry cone constraints turned on
        bineqvec(sum(con0(4:6))+1 : sum(con(4:7))) = -G1 * pdock_9;
        bineqvec(sum(con(4:7))+1 : sum(con(4:8))) = G2 * pdock_9;
    end
