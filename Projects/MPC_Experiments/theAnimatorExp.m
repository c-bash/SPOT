...
% ================================
filename = '4_12_15_6';
PRENAME = strcat('ExperimentData_RED_2022_',filename);
load(strcat(strcat('Saved Data/',PRENAME),'/MPC_Experiments_testing_1__stitched.mat'))
...
% for each component.
ani_posx = rt_dataPacket(startind:step:end,5);    % 1 x (k+1)
ani_posy = rt_dataPacket(startind:step:end,6);    % 1 x (k+1)
ani_theta = rt_dataPacket(startind:step:end,7);   % 1 x (k+1)
ani_velx = rt_dataPacket(startind:step:end,8);    % 1 x (k+1)
ani_vely = rt_dataPacket(startind:step:end,9);    % 1 x (k+1)
ani_omega = rt_dataPacket(startind:step:end,10)*180/pi; % converted to deg/s
ani_ux = rt_dataPacket(startind:step:end,2);      % 1 x (k+1)
ani_uy = rt_dataPacket(startind:step:end,3);      % 1 x (k+1)
ani_tau = rt_dataPacket(startind:step:end,4);     % 1 x (k+1)
...
ani_ptar = rt_dataPacket(startind:step:end,25:26); % (k+1) x 2
ani_targett = rt_dataPacket(startind:step:end,27);                % (k+1) x 1
ani_tar_velx = rt_dataPacket(startind:step:end,28);   % 1 x (k+1); m/s
ani_tar_vely = rt_dataPacket(startind:step:end,29);   % 1 x (k+1); m/s
ani_tar_omega = rt_dataPacket(startind:step:end,30)*180/pi; % 1 x (k+1); deg/s
...
for i = 1 : length(ani_posx)-1
    ...
    if i == length(ani_posx)-1
        % last still
        saveas(gcf,strcat(PRENAME,'still'))
        saveas(gcf,strcat(PRENAME,'still.png'))
    end
